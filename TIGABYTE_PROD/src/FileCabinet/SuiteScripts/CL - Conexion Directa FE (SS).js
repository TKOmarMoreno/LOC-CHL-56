...
define(
    [
        'N/record', 'N/error', 'N/search', 'L56/utilidades', 'N/runtime', 'N/format', 'N/config', 'N/file', 'N/render', 'N/url', 'N/https', 'N/transaction'
    ],
    function (record, error, search, utilities, runtime, format, config, file, render, url, https, transaction) {
                        ...
                        if (!utilities.isEmpty(docType)) {
                            if (!utilities.isEmpty(docXML)) {
                                try {
                                    ...
                                    formTransaction.clientScriptModulePath = './CL - Conexion Directa FE (CL).js';
        ...
        function beforeSubmit(scriptContext) {
            let process = 'beforeSubmit';
            let currentScript = runtime.getCurrentScript();
            let infoTransaction = scriptContext.newRecord;
            let observacionesExport = currentScript.getParameter('custscript_l56_conx_dirc_obs_fex'); // Observaciones para Factura de Exportacion 
            let facturaExportacionID = currentScript.getParameter('custscript_l56_conx_dirc_fex'); // Factura de Exportacion
            if (scriptContext.type == 'create' || scriptContext.type == 'copy' || scriptContext.type == 'edit') {
                //Observaciones Factura de Exportacion
                let docType = infoTransaction.getValue({ fieldId: 'custbody_zim_cl_tipo_doc_cod' });
                let obsFE = infoTransaction.getValue({ fieldId: 'custbody_l56_observaciones' });
                log.debug(process, "Obs Export" + JSON.stringify({observacionesExport ,facturaExportacionID,docType,obsFE}));
                if (facturaExportacionID == docType && utilities.isEmpty(obsFE)) {
                    infoTransaction.setValue('custbody_l56_observaciones', observacionesExport);
                }
            }
